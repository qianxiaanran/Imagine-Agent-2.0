...
const decisionApi = {
  getOverview: async ({ refreshAi = false, refreshData = false, backend = 'local' } = {}) => {
    const params = new URLSearchParams({
      refresh_ai: String(Boolean(refreshAi)),
      refresh_data: String(Boolean(refreshData)),
      analysis_backend: backend || 'local',
    });
  ...
  getAi: async ({ refreshAi = false, refreshData = false, backend = 'local' } = {}) => {
    const params = new URLSearchParams({
      refresh_ai: String(Boolean(refreshAi)),
      refresh_data: String(Boolean(refreshData)),
      analysis_backend: backend || 'local',
